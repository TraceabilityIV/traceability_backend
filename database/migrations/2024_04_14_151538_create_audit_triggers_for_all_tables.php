...
return new class extends Migration
{
    ...
    public function up(): void
    {
       ...
       BEGIN
           IF (TG_OP = \'INSERT\') THEN
               ...
               VALUES (TG_TABLE_NAME, NEW.id, NULL, row_to_json(NEW), \'create\', session_user, inet_client_addr(), current_timestamp);
               RETURN NEW;
           ELSIF (TG_OP = \'UPDATE\') THEN
               INSERT INTO audit_log (table_name, record_id, old_data, new_data, event, user_name, ip_address, created_at)
               VALUES (TG_TABLE_NAME, OLD.id, row_to_json(OLD), row_to_json(NEW), \'update\', session_user, inet_client_addr(), current_timestamp);
               RETURN NEW;
           ELSIF (TG_OP = \'DELETE\') THEN
               INSERT INTO audit_log (table_name, record_id, old_data, new_data, event, user_name, ip_address, created_at)
               VALUES (TG_TABLE_NAME, OLD.id, row_to_json(OLD), NULL, \'delete\', session_user, inet_client_addr(), current_timestamp);
               RETURN OLD;
           END IF;
           RETURN NULL;
       END;
   ...
   foreach ($tables as $table) {
        // Crear el trigger
        if($table->table_name != 'audit_log' && $table->table_name != 'migrations' && $table->table_name != 'failed_jobs' && $table->table_name != 'jobs' && $table->table_name != 'auditoria' && $table->table_name != 'notifications')
            DB::statement("
